...
class IMU {
  ...
  direction(): DirectionData {
    ...
    console.log(`Direction data: roll=${data.roll}, pitch=${data.pitch}, heading=${data.heading}`);
    return data;
  }
  ...
  raw(): RawData {
    // Replace with actual implementation
    const data: RawData = {
      accelerometer: {x: Math.random() * 10, y: Math.random() * 10, z: Math.random() * 10}, // Mock data
      compass: {x: Math.random() * 10, y: Math.random() * 10, z: Math.random() * 10} // Mock data
    };
    console.log(`Raw data: accelerometer=${JSON.stringify(data.accelerometer)}, compass=${JSON.stringify(data.compass)}`);
    return data;
  }
